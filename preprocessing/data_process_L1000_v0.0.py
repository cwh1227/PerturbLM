...
python data_process_L1000_v0.0.py \
...
def step_load_metadata(
    ...
    # ---- Dose ----
    cond_keep["dose"] = cond_keep["nearest_dose"].apply(safe_str)

    # ---- Time: append "h" unit ----
    t_col = cond_keep["pert_time"].apply(safe_str)
    cond_keep["time"] = t_col.where(t_col == "none", t_col + "h")
